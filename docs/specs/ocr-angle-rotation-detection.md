...
이 스펙의 신호는 **Vision이 observation quad를 이미지 공간 기준으로 보고한다**는 가정 하나에 전부 걸려 있다.
Vision이 자신이 읽어낸 방향을 기준으로 quad를 보고한다면 회전된 입력에서도 각도가 `0`으로 측정되고, 이 설계는 성립하지 않는다.
`RNOcrProcessor.m`의 주석은 이 가정을 소스로는 확인할 수 없는 유일한 가정으로 기록해 두었다.

측정 결과 가정은 참으로 확인되었다.
저장소의 `example/data/RECEIPT-{0,90,180,270}.jpg` 네 장을 프로덕션과 동일한 요청 설정(`VNRecognizeTextRequestRevision3`, `.accurate`, `["ko-KR", "en-US"]`, `usesLanguageCorrection = NO`, `minimumTextHeight = 1/32`)으로 인식했다.

| 입력 콘텐츠 | 라인 수 | 각도 bin `[0/90/180/270]` | `dominant` | `correction` | 보정 적용 후 재측정 |
| ----------- | ------- | ------------------------- | ---------- | ------------ | ------------------- |
| 0°          | 41      | `[40, 0, 0, 1]`           | `0`        | `0`          | `0` (정방향)        |
| 90°         | 38      | `[0, 38, 0, 0]`           | `90`       | `270`        | `0` (정방향)        |
| 180°        | 51      | `[0, 0, 51, 0]`           | `180`      | `180`        | `0` (정방향)        |
| 270°        | 38      | `[0, 0, 0, 38]`           | `270`      | `90`         | `0` (정방향)        |

최빈 bin의 점유율이 네 경우 모두 100%에 가까워 `ANGLE_MAJORITY = 0.7`을 큰 여유로 통과한다.
90°와 270°가 서로 다른 보정값을 받아 둘 다 정방향이 되었으므로, 이 재설계가 막으려던 90/270 부호 뒤바뀜도 발생하지 않는다.
180° 단독으로는 이 확인이 불가능하다는 점을 덧붙여 둔다.
`correction = 180`이면 시계방향과 반시계방향이 같은 값이라 방향 오류가 드러나지 않기 때문이다.

검증 절차 자체도 한 번 실패할 수 있는 형태로 구성했다.
정방향 이미지를 시계방향 90°로 돌린 뒤 각도를 재측정해 `90`이 나오는지 먼저 확인했고, 그 검사가 통과한 뒤에야 위 표의 결과를 신뢰했다.

**이 측정이 증명하지 않는 것.**
검증한 대상은 감지 결정과 회전 수식이며, 프로덕션이 반환하는 JPEG가 아니다.
`resultByRotating:`와 `RNImageProcessor`의 인코딩 경로는 여전히 미검증으로 남는다.
측정 환경도 iOS 실기기가 아니라 macOS Vision이므로, 두 플랫폼이 quad 좌표 규약을 공유한다는 전제가 하나 더 깔려 있다.
Android `Text.Line.getAngle()`은 측정되지 않았고, 근거는 여전히 ML Kit 문서와 `OcrGeometryTest`의 순수 수학 검증뿐이다.
...
**180° 단독 회전도 2026-07-19 2차 QA에서 확인됐다** (Android). `lineAspect`는 180°에서 정상 분포와 값이 같아 원리적으로 검출이 불가능했고 v1.3이 비범위로 명시한 항목인데, 각도는 `±180`을 그대로 내주므로 별도 분기 없이 해소됐다 — 이 설계가 예측한 대로다.

남은 미검증은 iOS의 0°·180°(2차 QA는 Android만 실행)와 임계값 보정용 로그 수집이다 (아래 DoD).
...
- [x] **180° 뒤집은 영수증 → 출력 이미지 정방향** (Android 2026-07-19) — v1.3이 비범위로 뒀던 신규 능력. iOS 미측정
- [x] 회전 적용 케이스에서 `ocrText` 라인 순서가 출력 이미지와 일치 (Android 90/270, 2026-07-19)
- [x] 정상 portrait(0°) → 회전 미적용, 회귀 없음 (Android 2026-07-19). iOS 미측정
- [ ] iOS에서 0° / 180° 확인 (Android만 실행됨)
- [ ] `angleBins` 로그 수집 → `ANGLE_MIN_LINES` / `ANGLE_MAJORITY` PROVISIONAL 해제
- [ ] 라인 부족(`lineCount < 5`) 입력 → 폴백 하강, 회귀 없음

### 2026-07-19 2차 QA (Android, `example/data/RECEIPT-*.jpg`)

| 입력 | 줄 수 | 신뢰도 | 출력 방향 | `ocrText` 첫 줄     |
| ---- | ----- | ------ | --------- | ------------------- |
| 90°  | 40    | 79.1%  | 정방향 ✅ | `emart24` (최상단)  |
| 270° | 44    | 78.0%  | 정방향 ✅ | `emart24` (최상단)  |
| 0°   | 42    | 78.1%  | 무회전 ✅ | `emart24` (최상단)  |
| 180° | 45    | 77.8%  | 정방향 ✅ | `emart 24` (최상단) |

네 케이스 모두 `ocrText`가 영수증 최상단에서 시작해 아래로 내려간다 — 회전 후 재인식이 의도대로 동작한다.

회전한 세 행(90°·270°·180°)의 `lineCount`는 재인식 패스 값이다 — JPEG 재인코딩을 한 번 더 거친 **출하 이미지** 기준이므로 검출 패스의 값과 다를 수 있다.
0° 행은 회전이 없어 재인식도 돌지 않으므로(§해소) 검출 패스 값 그대로다.
따라서 `ocrFloor.minLines`를 타이트하게 잡은 소비자가 영향을 받는 것도 회전 경로에 한정된다.
...
- **`ocrText` 라인 순서 (Android)** — 회전 후 재인식으로 수정함. 아래 별도 절 참조. Android는 2차 QA에서 확정, iOS 0°·180°는 미측정(아래 DoD).
...
✅ **2026-07-19 2차 QA에서 확정.** Android 90°·270° 모두 `ocrText`가 영수증 최상단(`emart24`)에서 시작한다 — 위 §2차 QA 표. iOS는 원래 이 경로였으므로 변경 없음.
