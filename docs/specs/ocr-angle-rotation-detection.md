...
`lineCount`가 이제 재인식 패스 값이라는 점에 주의한다. 위 숫자는 JPEG 재인코딩을 한 번 더 거친 **출하 이미지** 기준이므로 검출 패스의 값과 다를 수 있고, 이 값이 JS `OcrFloor` 게이트의 입력이다. `ocrFloor.minLines`를 타이트하게 잡은 소비자는 회전 경로에서 판정이 달라질 수 있다.
