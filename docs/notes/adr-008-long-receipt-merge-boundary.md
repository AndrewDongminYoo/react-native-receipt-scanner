...
- **`isComplete` is narrower than its name.** Both platforms can silently drop a page that fails to decode, and the JS layer cannot see that. The field means "everything that came back joined up". Closing this needs a native page-count signal and is deferred.
