...
**`isComplete` is narrower than its name, and says so.** Both iOS paths silently drop a page that fails to process — the camera delegate `continue`s past it and rejects only if every page failed; the gallery delegate appends only non-nil results — and JS cannot see it, so the field means "everything that came back joined up". Its JSDoc states the limitation in its first sentence rather than in a footnote.

This claim was wrong twice before it was right, which is worth recording. It was first written as a both-platform limitation; review showed Android rejects the whole scan instead of returning a partial batch, so it was narrowed to iOS gallery; review then showed the iOS _camera_ path drops pages the same way. Correcting an over-broad claim into an under-broad one is the same defect — the fix is to re-verify the new claim, not only to disprove the old one. The native page-count signal that would close the iOS gap is deferred — see the spec's §Follow-Ups.
